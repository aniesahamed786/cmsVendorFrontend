...
import { CreateRequestPayload, ApiRequestEntityType } from '../../../request-center/models/request-api.model';
...
export class EditBranch {
  ...
  private loadRequest(requestId: string): void {
    ...
    this.http
      ...
      .subscribe({
        next: (request:any) => {
          this.branchTitle = request?.title ?? '';
          this.entityId = request?.entityId ?? null;
          this.entityType = (request?.entityType as ApiRequestEntityType) || 'STORE';
          this.editableFormData.set(toEditableBranchData(request?.requestData));
          this.loading.set(false);
        },
        error: (err) => {
  ...
  private buildUpdatePayload(event: BranchFormSubmit, actionType: 'DRAFT' | 'SUBMIT'): CreateRequestPayload | null {
  ...
  private buildCreatePayload(event: BranchFormSubmit, actionType: 'DRAFT' | 'SUBMIT'): CreateRequestPayload {
    const requestData = getChangedFields(null, event.payload as unknown as Record<string, unknown>, {
      dropFiles: true,
    });
